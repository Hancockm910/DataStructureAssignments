...
struct DATA
{
	...
	{
		return (freq > d.freq); //Check if the direction is correct
	}
};
...
private:
	...
	NODE<TYPE>* _retrieve(KTYPE        key,
		NODE<TYPE>* root);
	NODE<TYPE>* _lowFreq(NODE<TYPE>* root,
		int        dltKey,
		bool& shorter,
		bool& success);
	NODE<TYPE>* _duplicate(KTYPE        key,
		NODE<TYPE>* root);

	void  _traversal(void (*process)(TYPE dataProc),
		NODE<TYPE>* root);

	void _findWord(KTYPE characters, string nextChar, NODE<TYPE>* root);


	//  	The following function is used for debugging.
	void  _print(NODE<TYPE>* root, int   level);

	priority_queue<TYPE> searchResult;
...
public:
	...
	bool  AVL_LowFreq(int  dltKey);
	bool  AVL_Duplicate(KTYPE  key, TYPE& dataOut);

	void AVL_findWord(KTYPE characters, string nextChar);
	...
	bool  AVL_Full(void);
	int   AVL_Count(void);
	priority_queue<TYPE> getSearchResult();
	//      The following function is used for debugging.
	void  AVL_Print(void);
...
template <class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::_findWord(KTYPE characters, string nextChar, NODE<TYPE>* root)
{
	if (root)
	{
		if (root->left)
		{
			if (root->left->data.key >= characters)
			{
				_findWord(characters, nextChar, root->left);
			}
		}
		if (root->data.key >= characters && root->data.key < nextChar)
		{
			searchResult.push(root->data);
		}
		if (root->right)
		{
			if (root->right->data.key < nextChar)
			{
				_findWord(characters, nextChar, root->right);
			}
		}
	}
}
...
template <class TYPE, class KTYPE>
std::priority_queue<TYPE> AvlTree<TYPE, KTYPE>::getSearchResult()
{
	return searchResult;
}

template <class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE>::AVL_findWord(KTYPE characters, string nextChar)
{
	while (!searchResult.empty())
	{
		searchResult.pop();
	}
	_findWord(characters, nextChar, tree);
	return;
}
...
{
	if (AVL_Duplicate(dataIn.key, dataIn))
	{
		return true;
	}
	//	Local Definitions 
...
template <class TYPE, class KTYPE>
bool  AvlTree <TYPE, KTYPE> ::AVL_LowFreq(int  dltKey)
{
	//	Local Definitions 
	bool shorter;
	bool success;
	...
	//	Statements 
	newRoot = _lowFreq(tree, dltKey, shorter, success);
	if (success)
	{
		tree = newRoot;
		count--;
	...
	bool& success)
{
	...
	if (root)
	{
		if (dltKey > root->data.freq)
		{
			root = _delete(root, root->data.key, shorter, success);
			count--;
			root = _lowFreq(root, dltKey, shorter, success);
		}
	}
	...
	if (root)
	{
		if (root->left)
		{
			root->left = _lowFreq(root->left, dltKey,
				shorter, success);
		}
	
		if (root->right)
		{
			root->right = _lowFreq(root->right, dltKey,
				shorter, success);
		}
	}
...
template <class TYPE, class KTYPE>
bool   AvlTree<TYPE, KTYPE>
::AVL_Duplicate(KTYPE   key, TYPE& dataOut)
{
	//	Local Definitions
	NODE<TYPE>* node;
	...
	NODE<TYPE>* root)
{
	...
	{
		if (key < root->data.key)
			return _duplicate(key, root->left);
		else if (key > root->data.key)
			return _duplicate(key, root->right);
		else
			// Found equal key 
			return (root);
	} // if root 
...
template <class TYPE, class KTYPE>
void AvlTree<TYPE, KTYPE> :: AVL_Destroy(void)
{
	//	Statements 
	if (tree)
		_destroyAVL(tree);
	...
	int         level)
{
	...
	if (root)
	{
		...
		cout << " Frequency: " << root->data.freq << endl;
